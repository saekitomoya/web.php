...
class CountButton extends React.Component {
  ...
  constructor(props) {
    super(props);
      // 状態を保持するプロパティstate
    this.state = { count: 100, is_up: false };
  }
    ...
    return React.createElement(
      "button",
      {
        onClick: () => this.setState(
          {
            count : countDown(this.state.count)
            
          }
        )
      },

  //もしthis.state.is_upがfalseであれば　this.state.count - 1, trueであればthis.state.count + 1

                //this.state.countが0になった時にis_upはtrue, 100になった時にfalseになる
